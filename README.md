...
The result is `Installer\Windows\MarkdownEditor-1.0.0-offline.exe` (~54 MB). No `vc_redist` bootstrapper is needed — the CRT DLLs (`vcruntime140.dll`, `vcruntime140_1.dll`, `msvcp140.dll`) are bundled next to the exe.
...
```bat
Installer\Windows\MarkdownEditor-1.0.0-offline.exe install --default-answer --confirm-command TargetDir=C:/path/to/install
```
...
* **Native drag-and-drop DMG (default)** — use `--dmg` or
  ...
  `Installer/MacOS/MarkdownEditor-1.0.0.dmg`. The disk image contains the
...
* **Qt Installer Framework DMG (optional)** — use `--ifw` or
  ...
  `Installer/MacOS/MarkdownEditor-1.0.0-offline.dmg`. Set `IFW_BIN` if
...
open Installer/MacOS/MarkdownEditor-1.0.0.dmg
```
...
The result is `Installer/Linux/MarkdownEditor-1.0.0-offline.run`
(self-contained, ~60–90 MB). Install with:

```bash
chmod +x Installer/Linux/MarkdownEditor-1.0.0-offline.run
./Installer/Linux/MarkdownEditor-1.0.0-offline.run
```
