...
cmake --build --preset debug
./build/debug/FreebuffMarkdownEditor.app/Contents/MacOS/FreebuffMarkdownEditor
```
...
The result is `Installer\Windows\FreebuffMarkdownEditor-1.0.0-offline.exe` (~54 MB). No `vc_redist` bootstrapper is needed — the CRT DLLs (`vcruntime140.dll`, `vcruntime140_1.dll`, `msvcp140.dll`) are bundled next to the exe.

### What `windeploy.bat` stages

Into `Installer\Windows\packages\com.freebuff.markdowneditor\data` (gitignored):
...
2. Stages the `FreebuffMarkdownEditor.app` bundle in
   `Installer/MacOS/packages/com.freebuff.markdowneditor/data/`.
...
* **Native drag-and-drop DMG (default)** — use `--dmg` or
  ...
  `Installer/MacOS/FreebuffMarkdownEditor-1.0.0.dmg`. The disk image contains the
...
* **Qt Installer Framework DMG (optional)** — use `--ifw` or
  ...
  `Installer/MacOS/FreebuffMarkdownEditor-1.0.0-offline.dmg`. Set `IFW_BIN` if
  ...
  (`/Applications/FreebuffMarkdownEditor` by
  default).
...
support when configuring the build.
...
open Installer/MacOS/FreebuffMarkdownEditor-1.0.0.dmg
```

Ad-hoc signing is sufficient for local testing. Public distribution should
provide a Developer ID signature and notarization using the normal Apple
release process; `CODESIGN_IDENTITY` is passed through to `macdeployqt6`
when set.
...
tools/makeicns/         Generates resources/FreebuffMarkdownEditor.icns on macOS
src/
